...
var STABLE_TOKENS = map[string]bool{"WBNB": true}
var BASE_TOKENS = []string{"WBNB", "FAST","CAKE", "ETH", "SUSHI", "DODO", "COMP", "DAI",
	"BAND", "DOT", "LINK", "USDT", "UNI", "SXP", "USDC", "1INCH", "YFI", "AAVE", "SNX",
	"SafeMoon", "BSCPAD", "VENUS", "BAKE", "Spartan", "Waultswap"}
...
func (r *StatasSvc) refreshSwapPairInfos() {
	...
	for _, swapContract := range r.swapPairList {
		...
		if swapInfo.reserve0*swapInfo.reserve1 < r.config.ChainConfig.Reserve {
			continue
		}
		if err != nil {
	...
	tokenPrice[r.config.ChainConfig.BaseCoin] =
		tokePriceMetrics[r.config.ChainConfig.BaseCoin][r.config.ChainConfig.QuoteCoin].Price
	...
	cakeIns, err := abi.NewBep20(ethcmm.HexToAddress(r.config.ChainConfig.BEP20), r.bscClient)
	if err != nil {
